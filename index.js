...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5589981246187 :+55 89 246187\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nBem vindo ao grupo meu chapa< *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5589981246187@s.whatsapp.net'
                        /******ApiKey Input******/
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Onde está o Link, hum?')
		...
		if (args.length < 1) return reply ('*PARCEIROS*: \nEaxy: https://youtube.com/c/EAXYFF\nYamii: https://youtube.com/c/TioYamiiroivemsmpaki\nSaad: https://youtube.com/channel/UCtX3fzhC-teLUpaS7Xz8nKA\nKazuma: https://youtube.com/channel/UChRgBlar0IKGAsseuBeNRgA')
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Flw seu *Zé preguinho*👋', text) // ur cods
					}, 0)
                     break
			...
			case 'closegc':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter Figurinhas em imagens ❌')
